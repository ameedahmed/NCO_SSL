...
class TSPTrainer:
    ...
    def _train_one_knn_batch(self,episode,batch_size):
        ...
        with torch.no_grad():
            ...
            non_matching_rows = ~torch.all(self.env.solution == self.env.selected_node_list, dim=1)
            
            times_solution_changed = non_matching_rows.sum().item()
        ...
        return problems_knn,after_repair_complete_solution,times_solution_changed, times_better_solution_found_in_a_batch
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.trainer_params['epochs']+1):
            ...
            # Train
            average_initial_cost_of_dataset_before_RRC = (self.env._get_travel_distance_2(self.env.raw_data_nodes,self.env.raw_data_tours).flatten().sum())/self.trainer_params['train_episodes']
            self.logger.info('Epoch {:3d}/{:3d}: Average Initial Cost of Dataset: {:.4f}'.format(epoch, self.trainer_params['epochs'],average_initial_cost_of_dataset_before_RRC))
            
            train_score, train_student_score, train_loss,total_times_in_1_epoch_solution_changed,total_times_in_1_epoch_better_solution_found = self._train_one_epoch(epoch)
            average_cost_of_dataset_after_RRC = (self.env._get_travel_distance_2(self.env.raw_data_nodes,self.env.raw_data_tours).flatten().sum())/self.trainer_params['train_episodes']
            self.logger.info('Epoch {:3d}/{:3d}: Average Cost of Dataset After RRC: {:.4f}'.format(epoch, self.trainer_params['epochs'],average_cost_of_dataset_after_RRC))
            
            self.result_log.append('train_score', epoch, train_score)
            self.result_log.append('train_student_score', epoch, train_student_score)
            self.result_log.append('train_loss', epoch, train_loss)
            self.result_log.append('total_times_in_1_epoch_solution_changed', epoch, total_times_in_1_epoch_solution_changed)
            ...
            if epoch > 1:  # save latest images, every epoch
                ...
                util_save_log_image_with_label(image_prefix, self.trainer_params['logging']['log_image_params_3'],
                                    self.result_log, labels=['total_times_in_1_epoch_solution_changed'])
                util_save_log_image_with_label(image_prefix, self.trainer_params['logging']['log_image_params_4'],
                                    self.result_log, labels=['total_times_in_1_epoch_better_solution_found'])
            ...
            if all_done or (epoch % img_save_interval) == 0:
                ...
                util_save_log_image_with_label(image_prefix, self.trainer_params['logging']['log_image_params_2'],
                                    self.result_log, labels=['train_loss'])
                #util_save_log_image_with_label(image_prefix, self.trainer_params['logging']['log_image_params_3'],
                                    #self.result_log, labels=['times_solution_changed_epochs_wise'])
    ...
    def _train_one_epoch(self, epoch):
        ...
        episode = 0
        loop_cnt = 0
        total_times_in_1_epoch_solution_changed= 0
        ...
        while episode < train_num_episode:
            ...
            self.problems_knn,self.abc,times_solution_changed,times_better_solution_found_in_a_batch = self._train_one_knn_batch(episode,batch_size)
            ...
            total_times_in_1_epoch_solution_changed = total_times_in_1_epoch_solution_changed + times_solution_changed
            ...
            loop_cnt += 1
            self.logger.info('Epoch {:3d}: Train {:3d}/{:3d}({:1.1f}%)  Score: {:.4f}, Score_studetnt: {:.4f},  Loss: {:.4f},Times Solution Changed/Batch: {:3d},Times Solution Improved/Batch: {:3d} '
                             .format(epoch, episode, train_num_episode, 100. * episode / train_num_episode,
                                     score_AM.avg, score_student_AM.avg, loss_AM.avg,times_solution_changed,times_better_solution_found_in_a_batch))

        # Log Once, for each epoch
        self.logger.info('Epoch {:3d}: Train ({:3.0f}%)  Score: {:.4f}, Score_studetnt: {:.4f}, Loss: {:.4f},Times Solution Changed/Epoch: {:3d},Times Solution Improved/Epoch: {:3d} '
                         .format(epoch, 100. * episode / train_num_episode,
                                 score_AM.avg, score_student_AM.avg, loss_AM.avg,total_times_in_1_epoch_solution_changed,total_times_in_1_epoch_better_solution_found))
            
        return score_AM.avg, score_student_AM.avg, loss_AM.avg,total_times_in_1_epoch_solution_changed,total_times_in_1_epoch_better_solution_found
    ...
    def _train_one_batch(self, episode,batch_size,epoch,solution,problems_knn):
        ...
        while not done:
            if current_step == 0:
                ...
                selected_teacher, prob, probs, selected_student = self.model(problems_knn, self.env.selected_node_list, solution, current_step,mode='train')  # 更新被选择的点和概率
