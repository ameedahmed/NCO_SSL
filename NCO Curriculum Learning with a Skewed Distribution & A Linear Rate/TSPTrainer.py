...
class TSPTrainer:
    ...
    def run(self):
        ...
        for epochs in range(self.start_epoch, self.trainer_params['epochs']+1):
            ...
            print(f'Problem Size in Append Section {self.env.problem_size}')
            self.result_log.append('problem_size',epochs,self.env.problem_size)
            self.result_log.append('training_time',epochs,training_time_epoch)
            self.scheduler.step()

            ############################
            # Logs & Checkpoint
            ############################
            self.logger.info(f" Total training  time for {epochs} with problem size {self.env.problem_size}: {training_time_epoch:.2f} seconds")
            ...
            if epochs > 1:  # save latest images, every epoch
                ...
                util_save_log_image_with_label(image_prefix, self.trainer_params['logging']['log_image_params_3'],
                                    self.result_log, labels=['problem_size'])
