...
curriculum_params ={
    'min_problem_size':20, #starting the TSP instances of 20 cities
    'max_problem_size': 100, #ending the TSP instances with 100 cities
    'size_increase_epochs': 1, #increasing the problem size every 10 epochs
    'schedule_type': 'linear',  # can be 'linear', 'sigmoid'
    'schedule_end_epoch': 100  # end of schedule before the last epoch (only applies to linear/sigmoid)
}

##########################################################################################
# main

def main_test(epoch, path,use_RRC=None,cuda_device_num=None,curriculum_params=curriculum_params):
    if DEBUG_MODE:
        _set_debug_mode()
