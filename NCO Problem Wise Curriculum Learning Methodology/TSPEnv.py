...
class TSPEnv:
    def __init__(self, **env_params):
        ...
        self.episode = None
        self.problem_size = env_params.get('problem_size', 20)

    def load_problems(self, episode, batch_size, epoch= None, curriculum_params=None,schedule_type='linear'):
        self.episode = episode
        self.batch_size = batch_size
        
        if curriculum_params and epoch is not None:
            schedule_type = curriculum_params.get('schedule_type','sigmoid')
            max_problem_size = curriculum_params['max_problem_size']
            min_problem_size = curriculum_params['min_problem_size']
            size_increase_epochs = curriculum_params['size_increase_epochs']
            schedule_end_epoch = curriculum_params.get('schedule_end_epoch', size_increase_epochs)
            
            if schedule_type == 'linear':
                print('Resorting to Linear loading')
                print(f'Epoch is {epoch}')
                #Linear Increase in Problem Size
                problem_size = min(min_problem_size + (epoch // size_increase_epochs) * 10, max_problem_size)
                
            elif schedule_type == 'sigmoid':
                print('Resorting to sigmoid loading')
                #Sigmoid Increase in Problem Size
                t=min(epoch/schedule_end_epoch,1)
                m = nn.Sigmoid()
                sigmoid_value = m(torch.tensor([12 * (t - 0.5)]))  # Scale sigmoid to be between 0 and 1
                problem_size = int(min_problem_size + (max_problem_size - min_problem_size) * sigmoid_value.item())
                
            else:
                print('Curr and Epoch is None')
                problem_size=min_problem_size
            
            #calculate the current problem size based on the curr
            #problem_size = min(
            #    curriculum_params['min_problem_size'] + (epoch // curriculum_params['size_increase_epochs']) * 10,
            #    curriculum_params['max_problem_size']
            #)
        else:
            print('Resorting to simple loading')
            problem_size = self.problem_size #default
            
        self.problems, self.solution = self.raw_data_nodes[:problem_size], self.raw_data_tours[:problem_size]
        ...
        if self.sub_path:
            self.problems, self.solution = self.sampling_subpaths(self.problems, self.solution,mode='train',max_n=problem_size)
    ...
    def sampling_subpaths(self, problems, solution, length_fix=False, mode='test', repair=False,max_n=100):
        ...
        # length of subpath: uniform sampling, from 4 to N, or max_n if specified
        if max_n is None:
            max_n = 100
            
        if mode == 'test':

            length_of_subpath = torch.randint(low=4, high=min(problems_size + 1, max_n), size=[1])[0]  # in [4,N]
        else:
            if length_fix:
                length_of_subpath = min(problems_size, max_n)
            else:
                alpha = 5.0
                beta=2.0
                ...
                low = 4
                high = min (problems_size+1,max_n+1)
                scaled_sample = low + (high-low)*sample
                length_of_subpath = torch.round(scaled_sample).int()
                print('Problem Size is {}'.format(problems_size))
                print(f'Currently Loaded Problem Size: {max_n}')
                print(f'Length of subpath {length_of_subpath}')
                """print(f'Problem Size is {problems_size}')
                print(f'Problem Size is {max_n}')
                length_of_subpath = torch.randint(low=4, high=min(problems_size + 1, max_n+1), size=[1])[0]  # in [4,N]
                print(f'Length of subpath {length_of_subpath}')"""
        # -----------------------------
        # new_sulution
        # -----------------------------
