...
class TSPEnv:
    ...
    def step(self, selected, selected_student,vanilla_mode=False,knnprob=None,sol=None):
        ...
        if vanilla_mode==True:
            done = (self.selected_count == knnprob.shape[1])
            if done:
                reward, reward_student = self._get_travel_distance(vanilla_mode=True,knn_prob=knnprob,sol=sol)
            else:
                reward, reward_student = None, None
        else:
            done = (self.selected_count == self.problems.shape[1])
            if done:
                reward, reward_student = self._get_travel_distance(vanilla_mode=False)
            else:
                reward, reward_student = None, None

        return self.step_state, reward, reward_student, done

    def simulated_step(self, selected, beam_node_list, problems):

        updated_node_list = torch.cat((beam_node_list, selected[:, None]), dim=1)

        # Compute incremental reward for this step
        prev_node = beam_node_list[:, -1] # Last Selected Node
        new_node = selected # Newly Selected Node

        # Extract coordinates
        prev_coords = problems.gather(1, prev_node.unsqueeze(1).unsqueeze(2).expand(-1, 1, 2)).squeeze(1)
        new_coords = problems.gather(1, new_node.unsqueeze(1).unsqueeze(2).expand(-1, 1, 2)).squeeze(1)

        incremental_reward = ((new_coords - prev_coords) ** 2).sum(1).sqrt()

        return updated_node_list, incremental_reward
    ...
    def _get_travel_distance(self,vanilla_mode=False,knn_prob=None,sol=None):
        if vanilla_mode == False:
            ...
            gathering_index = sol.unsqueeze(2).expand(self.batch_size, knn_prob.shape[1], 2)
            seq_expanded = knn_prob
            ordered_seq = seq_expanded.gather(dim=1, index=gathering_index)
            rolled_seq = ordered_seq.roll(dims=1, shifts=-1)
            segment_lengths = ((ordered_seq - rolled_seq) ** 2)
            segment_lengths = segment_lengths.sum(2).sqrt()
            ...
            # trained model's distance
            gathering_index_student = self.selected_student_list.unsqueeze(2).expand(-1, knn_prob.shape[1], 2)
            ordered_seq_student = knn_prob.gather(dim=1, index=gathering_index_student)
            rolled_seq_student = ordered_seq_student.roll(dims=1, shifts=-1)
            segment_lengths_student = ((ordered_seq_student - rolled_seq_student) ** 2)
            segment_lengths_student = segment_lengths_student.sum(2).sqrt()
            # shape: (batch,problem)
