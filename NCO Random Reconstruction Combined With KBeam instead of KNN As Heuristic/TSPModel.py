...
class TSPModel(nn.Module):
    ...
    def forward(self, state, selected_node_list, solution, current_step, beam_width, beam_search = False, repair = False, mode='test'):
        ...
        if self.mode == 'test':
            ...
            if  repair == True & beam_search == False:
                if current_step <= 2:
                    self.encoded_nodes = self.encoder(state.data)
                ...
                selected_teacher = selected_student
                prob = 1
            if repair == True & beam_search == True: #--> Beam search to be integrated here
                if current_step <= 2:
                    self.encoded_nodes = self.encoder(state.data)

                probs = self.decoder(self.encoded_nodes, selected_node_list)
                # THE PROBLEM LENGTH BECOMES A PROBLEM. IF WE RETURN THE TOP 3 PROBS AFTER THEY'VE BEEN SET TO -2 #
                # E.G. BEAM WIDTH IS 3: AT INDEX 22 WE HAVE JUST TWO VALUES LEFT
                top_n_probs, top_n_indices = torch.topk(probs, beam_width, dim=1)
                selected_students = top_n_indices
                selected_teachers = torch.gather(solution, 1, top_n_indices)
                prob = top_n_probs
        return selected_teacher, prob, 1, selected_student, selected_students, selected_teachers # MTW: Beam Search Variable here
...
class TSP_Decoder(nn.Module):
    ...
    def _get_new_data(self, data, selected_node_list, prob_size, B_V):
        ...
        # Filter greater than -1, and view it as a new tensor.
