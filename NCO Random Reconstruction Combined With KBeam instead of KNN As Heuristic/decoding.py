...
from utils_bqnco.misc import compute_tour_lens
...
def create_distance_matrix(self, nodes): # MW Implement - creating distance matrix for the beam search cost calc.
        # Creates a distance matrix for the given nodes (2D coordinates).
    nodes = nodes.float()
    dist_matrix = torch.cdist(nodes, nodes, p=2)
    return dist_matrix

def prepare_batch(self, data, sample=True):
    # Assume data is a tensor where the last part of each row contains the tour output
    ks = "_s" if sample else ""

    # Separate node coordinates (input) from the tour output sequence (target)
    node_coords = data[:, :-100].to(self.device)  # Assuming the last 100 columns are the tour output indices
    tour_output = data[:, -100:].to(self.device)  # Last 100 columns are the tour output sequence

    # Create distance matrix from node coordinates
    dist_matrices = self.create_distance_matrix(node_coords).to(self.device)  # Using the distance matrix function

    # Optionally, calculate tour lengths if needed
    init_tour_len = self.calculate_tour_length(tour_output)  # Placeholder for tour length calculation

    # Return the node coordinates, distance matrix, and tour output
    return node_coords, dist_matrices, tour_output, init_tour_len
...
def beam_search_decoding_step(sub_problem: DecodingSubPb, net: Module, prev_probabilities: Tensor, test_batch_size: int,beam_size: int, knns: int) -> (Tensor, DecodingSubPb):
