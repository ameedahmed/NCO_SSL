...
class TSPTrainer:
    ...
    def run(self):
        ...
        total_times_model_updated_in_this_run = 0
        ...
        for epoch in range(self.start_epoch, self.trainer_params['epochs']+1):
            ...
            train_score, train_student_score, train_loss,total_times_model_updated_per_epoch = self._train_one_epoch(epoch)
            current_time_epoch = time.time()
            training_time_epoch = current_time_epoch - self.start_time_epoch
            
            total_times_model_updated_in_this_run += total_times_model_updated_per_epoch
            ...
            # Logs & Checkpoint
            ############################
            self.logger.info(f" Total training  time for {epoch} Epoch with problem size {self.env.problem_size}: {training_time_epoch:.2f} seconds")
            
            elapsed_time_str, remain_time_str = self.time_estimator.get_est_string(epoch, self.trainer_params['epochs'])
            self.logger.info("Epoch {:3d}/{:3d}: Time Est.: Elapsed[{}], Remain[{}]".format(
                epoch, self.trainer_params['epochs'], elapsed_time_str, remain_time_str))
            
            self.logger.info(f'Times Model has been updated so far till epoch number {epoch:3d} is: {total_times_model_updated_in_this_run:3d}')
        ...
        self.logger.info("Total Times Model Was Updated In This Run: {:3d}',".format(total_times_model_updated_in_this_run))
    ...
    def _train_one_epoch(self, epoch):
        ...
        episode = 0
        loop_cnt = 0
        total_times_model_updated_per_epoch = 0
        while episode < train_num_episode:

            remaining = train_num_episode - episode
            batch_size = min(self.trainer_params['train_batch_size'], remaining)

            avg_score,score_student_mean, avg_loss,model_updated_in_batch = self._train_one_batch(episode,batch_size,epoch)
            
            total_times_model_updated_per_epoch += model_updated_in_batch
            ...
            loop_cnt += 1
            self.logger.info('Epoch {:3d}: Train {:3d}/{:3d}({:1.1f}%)  Score: {:.4f}, Score_studetnt: {:.4f},  Loss: {:.4f}, Times Model has been updated in the epoch so far {:3d}'
                             .format(epoch, episode, train_num_episode, 100. * episode / train_num_episode,
                                     score_AM.avg, score_student_AM.avg, loss_AM.avg,total_times_model_updated_per_epoch))

        # Log Once, for each epoch
        self.logger.info('Epoch {:3d}: Train ({:3.0f}%)  Score: {:.4f}, Score_studetnt: {:.4f}, Loss: {:.4f}, Total Times Model was updated in this epoch {:3d}'
                         .format(epoch, 100. * episode / train_num_episode,
                                 score_AM.avg, score_student_AM.avg, loss_AM.avg,total_times_model_updated_per_epoch))

        return score_AM.avg, score_student_AM.avg, loss_AM.avg, total_times_model_updated_per_epoch
    ...
    def _train_one_batch(self, episode,batch_size,epoch):
        ...
        return 0,0, loss_mean.item(), model_updated_in_batch